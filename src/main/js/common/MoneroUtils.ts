 ...
 */
class MoneroUtils {
  static PROXY_TO_WORKER: boolean = false;
  static NUM_MNEMONIC_WORDS = 25;
  static RING_SIZE = 12;
  static MAX_REQUESTS_PER_SECOND = 50;
  static AU_PER_XMR = 1000000000000;
   ...
   */
  static async validateMnemonic(mnemonic: string) {
    ...
    if (words.length !== MoneroUtils.NUM_MNEMONIC_WORDS)
      throw new MoneroError(
        "Mnemonic phrase is " +
          words.length +
          " words but must be " +
          MoneroUtils.NUM_MNEMONIC_WORDS
      );
  }
   ...
   */
  static async isValidPrivateViewKey(privateViewKey: string): Promise<boolean> {
    try {
   ...
   */
  static async isValidPublicViewKey(publicViewKey: string): Promise<boolean> {
    try {
   ...
   */
  static async isValidPrivateSpendKey(
   ...
   */
  static async isValidPublicSpendKey(publicSpendKey: string): Promise<boolean> {
    try {
   ...
   */
  static async validatePrivateViewKey(privateViewKey: string) {
    if (!MoneroUtils._isHex64(privateViewKey))
      throw new MoneroError(
        "private view key expected to be 64 hex characters"
      );
  }
   ...
   */
  static async validatePublicViewKey(publicViewKey: string) {
   ...
   */
  static async validatePrivateSpendKey(privateSpendKey: string) {
    if (!MoneroUtils._isHex64(privateSpendKey))
      throw new MoneroError(
        "private spend key expected to be 64 hex characters"
      );
  }
   ...
   */
  static async validatePublicSpendKey(publicSpendKey: string) {
    if (!MoneroUtils._isHex64(publicSpendKey))
      throw new MoneroError(
        "public spend key expected to be 64 hex characters"
      );
  }
   ...
   */
  static async getIntegratedAddress(
    networkType: string,
    ...
    if (MoneroUtils.PROXY_TO_WORKER)
      return new MoneroIntegratedAddress(
        await LibraryUtils.instance.invokeWorker(
          undefined,
          "moneroUtilsGetIntegratedAddress",
    ...
    if (LibraryUtils.instance.WASM_MODULE === undefined)
      await LibraryUtils.instance.loadKeysModule();

    // get integrated address in queue
    return LibraryUtils.instance.WASM_MODULE.queueTask(async function () {
      const integratedAddressJson =
        LibraryUtils.instance.WASM_MODULE.get_integrated_address_util(
          networkType,
          standardAddress,
          paymentId
        );
  ...
  static async validateAddress(
    ...
    if (MoneroUtils.PROXY_TO_WORKER)
      return LibraryUtils.instance.invokeWorker(
        undefined,
        "moneroUtilsValidateAddress",
        Array.from(arguments)
      );
    ...
    return LibraryUtils.instance.WASM_MODULE.queueTask(async function () {
      const errMsg = LibraryUtils.instance.WASM_MODULE.validate_address(
        address,
        networkType
      );
   ...
   */
  static async isValidPaymentId(paymentId: string): Promise<boolean> {
    try {
   ...
   */
  static async validatePaymentId(paymentId: string) {
   ...
   */
  static async getLastTxPubKey(txExtra: any[]): Promise<string> {
    let lastPubKeyIdx;
    ...
    return Buffer.from(
      new Uint8Array(txExtra.slice(lastPubKeyIdx, lastPubKeyIdx! + 32))
    ).toString("hex");
  }
  ...
  static paymentIdsEqual(paymentId1: string, paymentId2: string): bool {
    const maxLength = Math.max(paymentId1.length, paymentId2.length);
    for (let i = 0; i < maxLength; i++) {
      if (
        i < paymentId1.length &&
        i < paymentId2.length &&
        paymentId1[i] !== paymentId2[i]
      )
        return false;
   ...
   */
  static mergeTx(txs: MoneroTx[], tx: MoneroTx) {
   ...
   */
  static async jsonToBinary(json: object): Promise<Uint8Array> {
    if (MoneroUtils.PROXY_TO_WORKER)
      return LibraryUtils.instance.invokeWorker(
        undefined,
        "moneroUtilsJsonToBinary",
        Array.from(arguments)
      );
    ...
    return LibraryUtils.instance.WASM_MODULE.queueTask(async function () {
      ...
      const binMemInfoStr =
        LibraryUtils.instance.WASM_MODULE.malloc_binary_from_json(
          JSON.stringify(json)
        );

      // sanitize binary memory address info
      const binMemInfo = JSON.parse(binMemInfoStr);
      ...
      const view = new Uint8Array(binMemInfo.length);
      for (let i = 0; i < binMemInfo.length; i++) {
        view[i] =
          LibraryUtils.instance.WASM_MODULE.HEAPU8[
            binMemInfo.ptr / Uint8Array.BYTES_PER_ELEMENT + i
          ];
      }

      // free binary on heap
      LibraryUtils.instance.WASM_MODULE._free(binMemInfo.ptr);
   ...
   */
  static async binaryToJson(uint8arr: Uint8Array): Promise<object> {
    if (MoneroUtils.PROXY_TO_WORKER)
      return LibraryUtils.instance.invokeWorker(
        undefined,
        "moneroUtilsBinaryToJson",
        Array.from(arguments)
      );
    ...
    return LibraryUtils.instance.WASM_MODULE.queueTask(async function () {
      ...
      const ptr = LibraryUtils.instance.WASM_MODULE._malloc(
        uint8arr.length * uint8arr.BYTES_PER_ELEMENT
      );
      const heap = new Uint8Array(
        LibraryUtils.instance.WASM_MODULE.HEAPU8.buffer,
        ptr,
        uint8arr.length * uint8arr.BYTES_PER_ELEMENT
      );
      ...
      const binMemInfo = { ptr: ptr, length: uint8arr.length };

      // convert binary to json str
      const ret_string = LibraryUtils.instance.WASM_MODULE.binary_to_json(
        JSON.stringify(binMemInfo)
      );

      // free binary on heap
      LibraryUtils.instance.WASM_MODULE._free(ptr);
   ...
   */
  static async binaryBlocksToJson(uint8arr: Uint8Array): Promise<object> {
    if (MoneroUtils.PROXY_TO_WORKER)
      return LibraryUtils.instance.invokeWorker(
        undefined,
        "moneroUtilsBinaryBlocksToJson",
        Array.from(arguments)
      );
    ...
    return LibraryUtils.instance.WASM_MODULE.queueTask(async function () {
      ...
      // convert binary to json str
      const json_str = LibraryUtils.instance.WASM_MODULE.binary_blocks_to_json(
        JSON.stringify(binMemInfo)
      );
      ...
      const json = JSON.parse(json_str); // parsing json gives arrays of block and tx strings
      json.blocks = json.blocks.map((blockStr: any) => JSON.parse(blockStr)); // replace block strings with parsed blocks
      json.txs = json.txs.map((txs: any) =>
        txs ? txs.map((tx: any) => JSON.parse(tx.replace(",", "{") + "}")) : []
  ...
  static xmrToAtomicUnits(amountXmr: number | string): bigint {
    if (typeof amountXmr === "number") amountXmr = "" + amountXmr;
      ...
      throw new MoneroError(
        "Must provide XMR amount as a string or js number to convert to atomic units"
      );
    let decimalDivisor = 1;
    const decimalIdx = amountXmr.indexOf(".");
    if (decimalIdx > -1) {
      decimalDivisor = Math.pow(10, amountXmr.length - decimalIdx - 1);
      amountXmr =
        amountXmr.slice(0, decimalIdx) + amountXmr.slice(decimalIdx + 1);
    }
    return (
      (BigInt(amountXmr) * BigInt(MoneroUtils.AU_PER_XMR)) /
      BigInt(decimalDivisor)
    );
  }
  ...
  static atomicUnitsToXmr(amountAtomicUnits: bigint | string): number {
    if (typeof amountAtomicUnits === "string")
      ...
      throw new MoneroError(
        "Must provide atomic units as BigInt or string to convert to XMR"
      );
    // @ts-expect-error TS(2339): Property 'AU_PER_XMR' does not exist on type 'type... Remove this comment to see the full error message
    const quotientAndRemainder = amountAtomicUnits.divRem(
      BigInt(MoneroUtils.AU_PER_XMR)
    );
    return Number(
      quotientAndRemainder[0].toJSValue() +
        quotientAndRemainder[1].toJSValue() / MoneroUtils.AU_PER_XMR
    );
  }

  static _isHex64(str: any) {
