...
class HttpClient {
  ...
  private _DEFAULT_REQUEST = {
    method: "GET",
    requestApi: "fetch",
    resolveWithFullResponse: false,
    rejectUnauthorized: true,
  };
  private _PROMISE_THROTTLES: { [key: string]: any } = {};
  private _TASK_QUEUES: { [key: string]: any } = {};
  private _DEFAULT_TIMEOUT = 60000;
  private _MAX_TIMEOUT = 2147483647; // max 32-bit signed number
  private _HTTP_AGENT: http.Agent;
  private _HTTPS_AGENT: https.Agent;
  private _MAX_REQUESTS_PER_SECOND = 50;

  private constructor() {
    this._HTTP_AGENT = new http.Agent({ keepAlive: true });
    this._HTTPS_AGENT = new https.Agent({ keepAlive: true });
  }
   ...
   */
  async request(request: RequestOpts): Promise<ResponseOpts> {
    ...
    if (!this._TASK_QUEUES[defaultRequest.host])
      this._TASK_QUEUES[defaultRequest.host] = new ThreadPool(1);

    // initialize one promise throttle per host
    if (!this._PROMISE_THROTTLES[defaultRequest.host]) {
      this._PROMISE_THROTTLES[defaultRequest.host] = new PromiseThrottle({
        requestsPerSecond: this._MAX_REQUESTS_PER_SECOND,
    ...
    const requestPromise =
      defaultRequest.requestApi === "fetch"
        ? this._requestFetch(defaultRequest)
        : this._requestXhr(defaultRequest);
    ...
    return Promise.race([
      requestPromise,
      timeoutPromise,
    ]) as Promise<ResponseOpts>;
  }

  // ----------------------------- PRIVATE HELPERS ----------------------------

  async _requestFetch(
    req: RequestOpts & { host: string },
  ): Promise<ResponseOpts> {
    ...
    const opts: {
      ...
      uri: req.uri,
      body: req.body,
      agent: req.uri.startsWith("https") ? this._HTTP_AGENT : this._HTTPS_AGENT,
    ...
    const host = req.host;
    const self = this;
    const resp = await this._TASK_QUEUES[host].submit(async function (
      that: any,
    ) {
      return self._PROMISE_THROTTLES[host].add(
        function (opts: any) {
    ...
    // normalize response
    const normalizedResponse: Partial<ResponseOpts> = {};
    ...
    return normalizedResponse as ResponseOpts;
  }

  async _requestXhr(req: RequestOpts & { host: string }) {
    if (req.headers)
      ...
      return self._PROMISE_THROTTLES[host].add(
        function () {
          return new Promise(function (resolve, reject) {
            const digestAuthRequest = self.digestAuthRequest(
              method,
              uri,
              username,
              password,
    ...
    normalizedResponse.headers = this._parseXhrResponseHeaders(
  ...
  _parseXhrResponseHeaders(headersStr: string) {
    const headerMap: any = {};
